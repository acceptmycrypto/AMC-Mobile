...
export default class PostInfo extends React.Component {
  ...
  _shareMessage() {
    ...
    let link =
      'https://acceptmycrypto.herokuapp.com/feed/deals/' + this.state.deal_name;
    link = link.split(' ').join('%20');
    let check =
      'https://acceptmycrypto.herokuapp.com/feed/deals/Tech4Kids%20-%20Paw%20Patrol%20Soft%20Lite%20Figure';
        ...
        <ScrollView ref={scrollView => (this.scrollView = scrollView)}>
          ...
          >
            ...
            >
              <TouchableOpacity onPress={this._shareMessage}>
                <Text style={{ fontSize: 20 }}>Share Item</Text>
              </TouchableOpacity>
            </View>
          </View>
          {/* <View style={{ 
          ...
          {/* <View style={{marginTop: 20, flexDirection: 'row', justifyContent: 'space-between', borderBottomColor: '#dbd8ce',
            ...
            {this.state.checkedbox1 == false && <Text style={{color: 'red'}}>Please Select A Size</Text>}
              <Dropdown
                ...
                onChangeText={(value, index) =>
                  this.setState({ size: value, checkedbox1: true })
                }
                style={{ width: 100 }}
              />
            </View>
            ...
            <View style={{ marginRight: 20, width: 150 }}>
              {this.state.checkedbox2 == false && (
                <Text style={{ color: 'red' }}>Please select a color</Text>
              )}
              <Dropdown
                label="Select a color..."
                data={colors}
                onChangeText={(value, index) =>
                  this.setState({ color: value, checkedbox2: true })
                }
                style={{ width: 50 }}
              />
            </View>
